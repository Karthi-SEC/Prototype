import { edgeKey } from '../graph/roadGraph'
...
export function findPathAStarTimeWeighted({
  graph,
  startNodeId,
  goalNodeId,
  edgeCostSeconds,
  ...
  if (start === goal) {
    return { routeNodes: [start], edgeKeys: [], totalTimeSeconds: 0 }
  }
  ...
  const heuristicSeconds = (aId, bId) => {
    ...
    // Underestimates travel time by assuming max possible speed and no extra traffic delay.
  ...
  while (openSet.size > 0) {
    ...
    for (const { to: neighbor, edgeKey } of neighbors) {
      if (closedSet.has(neighbor)) continue

      const tentativeG = (gScore[current] ?? Infinity) + edgeCostSeconds(edgeKey, current, neighbor)
