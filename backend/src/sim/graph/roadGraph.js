...
function createDemoGraph({ size = 7 } = {}) {
  // Synthetic “city block” around a known lat/lon so Leaflet has sensible bounds.
  // (We can later swap this for OSM-derived graph.)
...
function routeEdgeKeysFromNodes(routeNodes) {
  const edgeKeys = []
...
function nearestNodeIdFromLatLng(graph, lat, lon) {
  let bestId = null
